/* In-page PDF samples. PDF.js is self-hosted and loaded only on demand. */
(() => {
  'use strict';
  ...
  async function loadSample(session) {
    ...
    try {
      ...
      session.loading = pdfjs.getDocument({
        url: new URL(`pdfs/${session.sample.file}`, document.baseURI).href,
      ...
      if (!current(session)) return;
      scroller.removeAttribute('aria-busy');
      status.textContent = 'We couldn’t load this sample. Please try again.';
      status.hidden = false;
      retry.hidden = false;
    }
  }
  ...
  function openSample(key, trigger) {
    ...
    dispose(active);
    active = { sample, trigger, oldOverflow, cancelled: false, records: [], tasks: new Set(), queue: [], busy: false, zoom: 1 };
    pages.style.width = '';
    pages.style.maxWidth = '';
    zoomValue.value = '100%';
    zoomOut.disabled = true;
    zoomIn.disabled = false;
    document.getElementById('mlPdfPreviewTitle').textContent = `${sample.subject} — ${sample.kind}`;
