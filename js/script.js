...
function applyAiAutomationPricing(countryCode) {
  ...
  ['ai-automation', 'data-science'].forEach(courseId => {
  ...
  if (typeof SEARCH_INDEX !== 'undefined') {
    SEARCH_INDEX.filter(item => ['Machine Learning Engineer Complete Kit', 'Data Scientist & Gen AI Complete Kit'].includes(item.name))
...
function openDiscountRequest(course) {
  ...
  document.getElementById('discountName').focus({preventScroll:true});
}
...
async function submitDiscountForm(event) {
  ...
  const message = 'Hi LearnLytics! I’m ' + name + '. I registered for a discount on the ' + course + '. Please share my coupon code, or payment link.';
  ...
  const body = new URLSearchParams({name,email,phone,
    source:'Register & Get Discount',course,reason});
