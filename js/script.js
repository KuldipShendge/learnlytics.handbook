...
const totalCourses = document.querySelectorAll('#track > .slide').length; 
...
function goToFreeML() {
  openModal('ds-handbook');
}

function openDetail(courseId) {
  // Retired Complete Kit links open the remaining DA Interview Kit.
  ...
  if (courseId === 'help') {
    ...
    const daNavItem = Array.from(document.querySelectorAll('.dashboard-nav-item'))
      .find(item => item.textContent.trim().startsWith('Data Analyst Interview Kit') || item.textContent.trim().startsWith('Data &amp; BI Analyst Kit'));
    ...
    const courseNavItem = Array.from(document.querySelectorAll('.dashboard-nav-item'))
      .find(item => item.textContent.includes('Dashboard') || item.textContent.includes('Course Handbook Bundle'));
  ...
  if (topbarLeft) {
    ...
    if (courseId === 'help') {
      ...
      const parentAction = courseId === 'data-analyst-questions' ? "openDetail('data-analyst')" : "openDetail('data-science')";
      const parentTitle = courseId === 'data-analyst-questions' ? 'Data &amp; BI Analyst Kit' : 'Data Scientist &amp; Gen AI Engineer';

      topbarLeft.innerHTML = `
        <div class="breadcrumbs" style="font-family:'Manrope', sans-serif;font-size:14px;font-weight:600;display:flex;align-items:center;">
          <span onclick="showDashboard()" style="cursor:pointer;color:var(--muted);transition:color 0.2s;">Home</span>
          <span style="color:var(--muted);margin:0 8px;">&gt;</span>
          <span onclick="${parentAction}" style="cursor:pointer;color:var(--muted);transition:color 0.2s;">${parentTitle}</span>
          <span style="color:var(--muted);margin:0 8px;">&gt;</span>
...
function showDashboard() {
  ...
  if (topbarLeft) {
    topbarLeft.innerHTML = `
      ...
      <p>Build the skills. Practise the questions. <strong>Prepare with confidence.</strong></p>
    `;
  }
...
var sectionConfig = {
  about: { id: 'aboutSection', title: 'About Us', subtitle: '' },
  register: { id: 'registerSection', title: 'Register & Get Discount', subtitle: '' },
...
function openModal(type) {
  ...
  if (window.freeHandbookAdditions && Object.prototype.hasOwnProperty.call(window.freeHandbookAdditions, type)) {
    const book = window.freeHandbookAdditions[type];
    currentModalConfig = {
      title: 'Free Download ' + book.title,
      sub: 'Enter your details to receive the free download link in your inbox.',
      successTitle: 'Your free download is ready!',
      successMsg: 'Use the link below to open your free handbook. The same link has been sent to your email.',
      downloadText: 'Download ' + book.title,
      fileLink: book.link,
      subject: 'Your Free ' + book.title + ' — Learnlytics.handbook',
      emailTitle: 'Free ' + book.title
    };
  } else if (type === 'sql-questions') {
   ...
   } else if (type === 'ml-free-kit') {
    currentModalConfig = {
      title: "Free Download Machine Learning Part-01 Handbook + Interview QS",
      sub: "Enter your details to get the free Part-01 handbook and interview questions together in one set.",
      successTitle: "Your free kit is ready!",
      successMsg: "Download your Part-01 handbook and interview questions using the single link below. The same link is included in your email.",
      downloadText: "Download Part-01 Handbook + Interview QS",
      fileLink: "https://drive.google.com/file/d/1UgDoS8ql9byHx2rtPORYe0bGSr2jwNaY/view?usp=sharing",
      subject: "Your Free ML Part-01 Handbook + Interview QS — Learnlytics.handbook",
      emailTitle: "Free Machine Learning Part-01 Handbook + Interview QS"
    };
  ...
  document.querySelector('#modal-success h3').textContent = currentModalConfig.successTitle || 'Check Your Email!';
...
function applyAiAutomationPricing(countryCode) {
  ...
  const aiRoot = document.getElementById('course-' + courseId);
  if (aiRoot) {
    aiRoot.querySelectorAll('.dakit-hero-pricing-new, .cta-price, .dakit-cta-pricing strong, .dakit-sticky-price strong')
      .forEach(el => el.textContent = price);
    aiRoot.querySelectorAll('.dakit-hero-pricing-old, .dakit-cta-pricing del, .dakit-sticky-price del')
      .forEach(el => el.textContent = strike);
    aiRoot.querySelectorAll('.dakit-hero-pricing-badge, .dakit-off-badge, .dakit-sticky-price .sticky-off')
      .forEach(el => el.textContent = discount);
  }

  });

  const checkoutLink = 'https://rzp.io/rzp/KKrZwPm';
  if (typeof mlKitConfig !== 'undefined') {
    mlKitConfig.kitDisplay = price;
    mlKitConfig.totalDisplay = price;
    mlKitConfig.kitLink = checkoutLink;
  ...
  if (typeof SEARCH_INDEX !== 'undefined') {
    SEARCH_INDEX.filter(item => ['Machine Learning Engineer Complete Kit'].includes(item.name))
      .forEach(item => item.price = price);
  }
}
...
function getScienceKitCheckout(courseId) {
  const kit = SCIENCE_KIT_CHECKOUT[courseId];
...
function applyScienceKitPricing(countryCode) {
  ...
  const international = scienceKitCountry !== 'IN';
  Object.entries(SCIENCE_KIT_CHECKOUT).forEach(([id, kit]) => {
    const price = international ? kit.usd : kit.inr;
    const root = document.getElementById('course-' + id);
    root?.querySelectorAll('.dakit-hero-pricing-new, .cta-price, .dakit-cta-pricing strong, .dakit-sticky-price strong').forEach(el => el.textContent = price);
    const card = document.querySelector('.course-bundle-card button[onclick*="' + id + '"]')?.closest('.course-bundle-card');
    const cardPrice = card?.querySelector('.bundle-price-large');
    if (cardPrice) cardPrice.textContent = price;
    if (id === 'data-science') {
      ...
      if (card?.querySelector('.bundle-discount-badge')) card.querySelector('.bundle-discount-badge').textContent = discount;
    }
    if (typeof SEARCH_INDEX !== 'undefined') SEARCH_INDEX.filter(item => item.name === kit.title).forEach(item => item.price = price);
  });
}
...
async function localizePrices() {
  ...
  applyScienceKitPricing(countryCode);
...
function openExploreAllHandbooks() {
  ...
  if (p) p.innerHTML = 'Build the skills. Practise the questions. <strong>Prepare with confidence.</strong>';
...
const SEARCH_INDEX = [
  {
    ...
    action: function() { openDetail('ds-genai-ml'); }
  },
  // -- Course Bundles --
  {
  ...
  {
    name: "Machine Learning Engineer Complete Kit",
  ...
  {
    name: "Data Scientist & Gen AI Complete Kit",
    keywords: ["data scientist", "data science", "ds", "gen ai", "genai", "generative ai", "interview kit", "course bundle"],
    category: "Course Bundle",
    categoryClass: "cat-bundle",
    price: "₹499",
    icon: "🤖",
  ...
  try {
    ...
    if (!checkPath || checkPath === '/' || checkPath === '/index.html') {
      if (window.location.hash) {
        ...
        } else if (hash === 'ds-genai-ml' || hash === 'ds-genai-ml-complete-kit') {
          checkPath = '/course-bundles/ds-genai-ml-complete-kit';
    ...
    if (
      ...
      openDetail('help');
    } else if (normalizedPath === '/course-bundles/ds-genai-ml-complete-kit') {
...
if (typeof openDetail === 'function') {
  ...
  openDetail = function(courseId, ...args) {
    ...
    if (courseId === 'data-analyst') {
      ...
      safePushState('/course-bundles/ds-genai-ml-complete-kit');
    } else if (courseId === 'data-science') {
...
(function initUniversalTestimonialsAutoScroll() {
  ...
  function setupAll() {
    ...
    setupTrackAutoScroll('dsTestiTrack');
  }
...
(function() {
  ...
  function updateCountdown() {
    ...
    ['Hrs', 'Mins', 'Secs', 'MobileMins', 'MobileSecs'].forEach(function(suffix) {
      var source = document.getElementById('ai' + suffix);
      var copy = document.getElementById('ds' + suffix);
      if (source && copy) copy.textContent = source.textContent;
    });
  }
...
(function initCROFeatures() {
  ...
  // --- Sticky Mobile CTA for DS Interview Kit ---
  var dsStickyBar = document.getElementById('dskitStickyMobileCta');
  var dsHeroSection = document.querySelector('#course-data-science .dakit-hero');

  if (dsStickyBar && dsHeroSection) {
    var dsStickyObserver = new IntersectionObserver(function(entries) {
      entries.forEach(function(entry) {
        var dsPage = document.getElementById('course-data-science');
        if (dsPage && dsPage.style.display !== 'none' && dsPage.classList.contains('active')) {
          dsStickyBar.classList.toggle('visible', !entry.isIntersecting);
        } else {
          dsStickyBar.classList.remove('visible');
        }
      });
    }, { threshold: 0.1 });
    dsStickyObserver.observe(dsHeroSection);
  }
...
function openDiscountRequest(course) {
  showDashboardSection('register');
  document.getElementById('discountForm').hidden = false;
  document.getElementById('discountSuccess').hidden = true;
  document.getElementById('discountError').hidden = true;
  document.getElementById('discountCourse').value = course || '';
  document.getElementById('discountPreviousCourse').focus({preventScroll:true});
}
...
async function submitDiscountForm(event) {
  ...
  const previousSelect = document.getElementById('discountPreviousCourse');
  const previousCourse = previousSelect.options[previousSelect.selectedIndex].text;
  const courseSelect = document.getElementById('discountCourse');
  const course = courseSelect.options[courseSelect.selectedIndex].text;
  const reason = document.getElementById('discountReason').value || 'Not specified';
  const error = document.getElementById('discountError');
  error.hidden = true;
  if (!name || !/^[+\d\s().-]+$/.test(phone) || phone.replace(/\D/g,'').length < 10 || phone.replace(/\D/g,'').length > 15) {
    error.textContent = 'Please enter your name and a valid WhatsApp number with country code.';
    error.hidden = false;
    return;
  }
  const message = 'Hi LearnLytics! I’m ' + name + '. I registered for a discount on the ' + course + '. Previous purchase: ' + previousCourse + '. Please share my available discount and payment link.';
  const chatLink = 'https://wa.me/919130631497?text=' + encodeURIComponent(message);
  // Dedicated discount-lead endpoint: save contact details without a download email.
  const body = new URLSearchParams({name,email,phone,
    source:'Register & Get Discount',previousCourse,course,reason});
  button.disabled = true;
  button.textContent = 'Submitting…';
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 20000);
  try {
    const response = await fetch('https://script.google.com/macros/s/AKfycbwNFB5OzO2ZT4i0c9oREhnFfhmdLxch-mXq7cmguPBLrgWr-SSk9zpybkloAUD_8InmlQ/exec', {method:'POST',body,signal:controller.signal});
    ...
    error.textContent = 'We couldn’t confirm your request. Please try again.';
    error.hidden = false;
  } finally {
    clearTimeout(timeout);
    button.disabled = false;
    button.textContent = 'Unlock My Discount →';
  }
}
